...
import { updateDefaultValues, getDefaultValues, getdata_drug, getdrugtypes_test } from "../Functions_Files/Fetchdata";
...
import DrugFactorSelector from "../Graphs/DrugTypeAgg";
...
const Drug = () => {
    ...
    const handleChange = (e) => {
        ...
        if (e.target.checked) message.info("Department wise drug count")
        else message.info("Drug count Aggregator")
    };
    ...
    const fetchData = async (initialLoad = false) => {
        ...
        try {
            ...
            const res = await getdata_drug(from_date, to_date, department, group, token, currentTypeString);
            const res_type = await getdrugtypes_test(token);
                ...
                <div className={Styles.down}>
                    ...
                    {showFullScreenChart && selectedChart !== 'None' ? (
                        selectedChart === "chart1" ? (
                            <FlexiblePlotlyChart
                                key={`chart-${themeKey}`}
                                data={data?.drug_order_count}
                                chartTitle="Drug count"
                                xAxisTitle="Time"
                                yAxisTitle="Count"
                            ...
                            <FlexiblePlotlyChart
                                key={`chart-${themeKey}`}
                                data={data?.drug_orders_by_department}
                                chartTitle="Department wise Drug count"
                                xAxisTitle="Time"
                                yAxisTitle="Count"
                            ...
                            <>
                                ...
                                <FlexiblePlotlyChart
                                    data={data?.monthly_drug_test_counts}
                                    key={`chart-${themeKey}`}
                            ...
                            <DrugFactorSelector testTypes={typetest.map(item => item.label)} />
                        ) : null
                    ) : (
                        <>
                                    ...
                                    <>
                                        {checked ?
                                            ...
                                            <FlexiblePlotlyChart
                                                ...
                                                chartTitle={"Department wise Drug"}
