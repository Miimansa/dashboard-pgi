...
import { getdrugData, getprocedureData } from '../Functions_Files/Fetchdata';
const { Option } = Select;
const { Title, Text } = Typography;


const DrugFactorSelector = ({ testTypes }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            if (selectedDepartments.length === 0 || selectedTestTypes.length === 0) {
                ...
                const res = await getdrugData(from_date, to_date, selectedDepartments.join(','), selectedTestTypes.join(','), factor, groupingType, token);
...
export default DrugFactorSelector;