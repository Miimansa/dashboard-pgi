...
import FlexiblePlotlyChart from "./FlexibleChart";
...
import { getprocedureData } from '../Functions_Files/Fetchdata';
const { Option } = Select;
const { Title, Text } = Typography;


const ProcedureFactorSelector = ({ testTypes }) => {
    ...
    const fetchData = async () => {
        ...
        try {
            if (selectedDepartments.length === 0 || selectedTestTypes.length === 0) {
                ...
                const res = await getprocedureData(from_date, to_date, selectedDepartments.join(','), selectedTestTypes.join(','), factor, groupingType, token);
    ...
    const getChartTitle = () => {
        ...
        switch (factor) {
            case 'department':
                line1 = `Department Procedure Wise Record Count`;
                line2 = `for ${testTypeInfo}`;
                break;
            case 'testType':
                line1 = `Procedure Wise Lab Record Count`;
                line2 = `for ${departmentInfo}`;
                break;
            default:
                line1 = `${factorName} Procedure Count`;
                line2 = '';
        }
        return [line1, line2];
    };
...
export default ProcedureFactorSelector;